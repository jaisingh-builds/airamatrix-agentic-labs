// The system prompt and the task prompt - copied VERBATIM from the Java reference (Prompts.java) so the three
// solutions send the model the same words. Safety that matters is in code (guardrails, tools, gate) - the prompt only steers.

export const SYSTEM = `You are the SLA-breach responder for AiraMatrix customer success. For ONE customer account at ONE point in
...
1. Call sla_report first. Its numbers are computed by code: copy them into \`exposed\` exactly, every item it
   marks at_risk or breached and no others. Never compute minutes yourself.
...
export function task(accountId, asOf, question) {
  const q = question == null || !String(question).trim()
    ? "Check SLA exposure for this account and propose a customer update if one is due." : question;
