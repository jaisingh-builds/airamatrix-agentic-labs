#!/usr/bin/env python3
"""
Lab 5.1 STARTER - identical to ../pipeline.py except the four TODO blocks, which are yours.
    LAB51_TARGET=starter python3 -m unittest test_pipeline
...
def run_stage(store, runner, tracer, rid, name, system, prompt, schema, extra_check=None):
    ...
    # Look the stage up in the store. If it already finished, return its stored
    # output without calling the runner (and emit a "stage.<name>.skipped" event).
    raise NotImplementedError("TODO 1: checkpoint - skip stages that already finished")
    # <<< TODO 1
...
def apply(store, rid, write_token, ops_url=OPS_URL, tracer=None):
    ...
    # Load the decision with store.approval(rid). No record, or not "approve":
    # raise GateError. Do not trust the status field on its own.
    a = None
    ...
    # Get the stored operation for this run; if there is none, create one with a
    # new uuid4 via store.record_operation(...) BEFORE any request is sent, then
    # reload it. A retry must reuse the stored op_id.
    op = None
    raise NotImplementedError("TODO 4: persist the operation id before sending")
    # <<< TODO 4
    if op["status"] == "done":
        return store.run(rid)
