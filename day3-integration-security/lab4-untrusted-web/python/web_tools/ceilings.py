...
def read_bounded(response, limit=MAX_BODY_BYTES, on_over="summarise"):
    ...
    # reported as truncated, and the caller chases a summary of a whole document.
    budget = limit + 1
    buf = bytearray()
    ...
    text = bytes(buf[:limit]).decode("utf-8", "replace")
    ...
    if on_over == "refuse":
        raise EgressRefusal(
            "byte_cap",
            "body exceeds the %d-byte read cap and this caller needs it whole. "
            "Fetch a narrower endpoint, or a filtered or paginated URL." % limit,
        )

    return _summarise(text, limit), True
...
class FetchBudget:
    ...
    def reserve(self, url=""):
        ...
        if self.used >= self.max_fetches:
            raise EgressRefusal(
                "fetch_cap",
                "fetch budget spent: %d of %d used this run%s. No further fetches "
                "will be made. Answer from what you already have, or say which "
                "single URL would settle it and why."
                % (self.used, self.max_fetches, (" (refused: %s)" % url) if url else ""),
            )
        self.used += 1
        return self.used
