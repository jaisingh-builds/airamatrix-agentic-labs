...
def wrap_untrusted(body, source, **meta):
    ...
    """
    # TODO 3 — the envelope. Every comment from this body is kept below.
    #
    ...
    # nonce is exactly as useful to an attacker as no nonce. Mint NONCE_BYTES of
    # it here, on every call.
    #
    # A meta key in _RESERVED_ATTRS must be refused: a second source= would
    # shadow the real origin in whatever reads the tag. Today the signature
    # catches that one first; refuse it anyway so the guarantee survives a later
    # slice widening the signature. A key that fails _SAFE_ATTR_NAME is refused
    # for the same reason — both raise ValueError.
    #
    # Escape every attribute value with _escape_attr, and _render meta values
    # first; the source URL is attacker-influenced.
    #
    ...
    #
    # The close tag carries the nonce too, or the body can close the envelope.
    raise NotImplementedError(
        "TODO 3: wrap_untrusted — return the body inside an <untrusted> envelope "
        "carrying a fresh per-call nonce the body cannot guess or close")
