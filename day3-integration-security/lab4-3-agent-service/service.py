...
What this lab is really about — the production concerns around the call:
  ...
  * cancellation              the user can stop it; checked while streaming
  * budget + fallback         a spend ceiling; on any stop, return what was found
...
from stream import stream_messages, StreamCancelled  # noqa: E402
...
class Run:
    ...
    def stopped(self):
        return self.cancel or time.monotonic() > self.deadline
    ...
    def work(self):
        ...
        try:
            while True:
                ...
                if self.budget.spent >= self.budget.limit:
                ...
                for attempt in (1, 2):
                    try:
                        reply = stream_messages(cfg.base_url, cfg.api_key,
