...
{"error": {"code": "outcome_unknown", "operation_id": "3f0c…", "retryable": true,
  "hint": "To retry THIS operation safely, call the tool again with operation_id=\"3f0c…\" …"}}
```

Passing that `operation_id` back retries the same operation; the server sends
it as the `Idempotency-Key` header, so it is applied at most once.

**Who holds the retry state?** In a service you own, the orchestrator records
`operation_id` and reuses it on retry itself — it must not depend on the model
remembering or copying it. Claude Code keeps no such state for an MCP tool, so
this lab relies on the model echoing `operation_id` from the error. That is a
stated limitation; payload binding keeps it safe. aira-ops binds each key to its
